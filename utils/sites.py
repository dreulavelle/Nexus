...
class SiteInfo(BaseModel):
    ...
    async def search(self, query: str, page: int, limit: int):
        ...
        if scraper_instance and hasattr(scraper_instance, "search"):
            return await scraper_instance.search(query, page, limit)
        else:
            raise NotImplementedError("Scraper not implemented for search.")
...
class Sites(BaseModel):
    ...
    def items(self) -> Dict[str, Union[str, SiteInfo]]:
        ...
        site_items = {}
        for site_name, site_instance in self.__dict__.items():
            if not site_name.startswith("_"):
                if hasattr(site_instance, "search"):
                    site_items[site_name] = site_instance
        return site_items
